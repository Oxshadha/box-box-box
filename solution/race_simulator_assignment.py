...
MODEL_DIR = ROOT / "analysis" / "models" / "assignment_model"
...
def predict_finishing_positions(test_case):
    ...
    race_rows = len(frame)
    cost = -np.log(np.clip(proba[:, :race_rows], 1e-9, 1.0))
    row_ind, col_ind = linear_sum_assignment(cost)
    assigned = pd.DataFrame(
        {
            "driver_id": frame.iloc[row_ind]["driver_id"].to_numpy(),
            "pred_rank": (col_ind + 1).astype(int),
        }
    )
    assigned = assigned.sort_values(["pred_rank", "driver_id"], ascending=[True, True])
    return assigned["driver_id"].tolist()
