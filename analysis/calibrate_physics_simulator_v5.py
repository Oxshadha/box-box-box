...
OUTPUT_DIR = ROOT / "analysis" / "models" / "physics_simulator_v5"
PARAMS_PATH = OUTPUT_DIR / "best_params.json"
METRICS_PATH = OUTPUT_DIR / "metrics.json"

COMPOUNDS = ("SOFT", "MEDIUM", "HARD")
LAP_BANDS = ("short", "mid_short", "mid", "long")
TEMP_BANDS = ("cool", "mild", "warm", "hot")
...
def predict_total_time(frame: pd.DataFrame, params: dict) -> np.ndarray:
    ...
    lap_one_hot = {
        band: (frame["lap_band"] == band).to_numpy(dtype=float)
        for band in LAP_BANDS
    }
    temp_one_hot = {
        band: (frame["temp_band"] == band).to_numpy(dtype=float)
        for band in TEMP_BANDS
    }

    pred += params["base_first_stop_frac"] * frame["first_stop_frac"].to_numpy(dtype=float)
    pred += params["base_second_stop_frac"] * frame["second_stop_frac"].to_numpy(dtype=float)
    pred += params["base_final_stint_frac"] * frame["stint_3_frac"].to_numpy(dtype=float)

    for band in LAP_BANDS:
        pred += params[f"lapband_{band}_bias"] * lap_one_hot[band]

    for band in TEMP_BANDS:
        pred += params[f"tempband_{band}_bias"] * temp_one_hot[band]
    ...
    for compound in COMPOUNDS:
        ...
        compound_frac = laps / frame["total_laps"].to_numpy(dtype=float)
        ...
        pred += params[f"{prefix}_frac_bias"] * compound_frac

        for band in LAP_BANDS:
            pred += params[f"{prefix}_lapband_{band}"] * lap_one_hot[band] * laps
        for band in TEMP_BANDS:
            pred += params[f"{prefix}_tempband_{band}"] * temp_one_hot[band] * laps
...
def sample_params(rng: np.random.Generator) -> tuple[dict, dict, dict]:
    ...
    params = {
        "base_first_stop_frac": rng.uniform(-2.0, 2.0),
        "base_second_stop_frac": rng.uniform(-1.0, 1.0),
        "base_final_stint_frac": rng.uniform(-1.5, 1.5),
    }

    for band in LAP_BANDS:
        params[f"lapband_{band}_bias"] = rng.uniform(-2.0, 2.0)
    for band in TEMP_BANDS:
        params[f"tempband_{band}_bias"] = rng.uniform(-1.0, 1.0)

    base_ranges = {
        "soft": (-2.2, -0.2, 0.0, 0.05, 0.08, 0.30, 0.001, 0.010, 0.0005, 0.0060, 0.05, 0.35, -1.5, 1.5),
        "medium": (-1.0, 0.8, 0.0, 0.04, 0.04, 0.18, 0.0005, 0.007, 0.0002, 0.0040, 0.03, 0.20, -1.5, 1.5),
        "hard": (0.0, 1.8, -0.01, 0.03, 0.02, 0.12, 0.0000, 0.005, 0.0000, 0.0030, 0.01, 0.12, -1.5, 1.5),
    }
    for prefix, offset_lo, offset_hi, fresh_lo, fresh_hi, wear_lo, wear_hi, quad_lo, quad_hi, temp_lo, temp_hi, cliff_lo, cliff_hi, frac_lo, frac_hi in (
        ("soft", *base_ranges["soft"]),
        ("medium", *base_ranges["medium"]),
        ("hard", *base_ranges["hard"]),
    ):
        ...
        params[f"{prefix}_frac_bias"] = rng.uniform(frac_lo, frac_hi)
        for band in LAP_BANDS:
            params[f"{prefix}_lapband_{band}"] = rng.uniform(-0.08, 0.08)
        for band in TEMP_BANDS:
            params[f"{prefix}_tempband_{band}"] = rng.uniform(-0.08, 0.08)

    return params, thresholds, cliff_thresholds


def calibrate(train_base: pd.DataFrame, validation_base: pd.DataFrame, iterations: int = 60, seed: int = 42):
...
def main():
    ...
    sampled_train_races = train["race_id"].drop_duplicates().sample(n=300, random_state=42)
    sampled_validation_races = validation["race_id"].drop_duplicates().sample(n=120, random_state=42)
