...
OUTPUT_DIR = ROOT / "analysis" / "models" / "balanced_tail_rules"
...
def route_balanced(frame):
    ...
    keep = set(race_level.loc[balanced_cluster, "race_id"])
...
def main():
    ...
    metadata, ranker = load_balanced_bundle()
    ...
    balanced = route_balanced(df)

    train = balanced[balanced["race_id"].isin(splits["train"])].copy()
    validation = balanced[balanced["race_id"].isin(splits["validation"])].copy()
    ...
    summary = {
        ...
        "validation_balanced_races": int(validation["race_id"].nunique()),
    }
