...
OUTPUT_DIR = ROOT / "analysis" / "models" / "physics_simulator_v2"
...
def stint_contribution(laps: np.ndarray, threshold: int):
    ...
    total_age_sum = laps * (laps + 1.0) / 2.0
    ...
    # Excess age starts once the fresh window ends.
    excess_age_sum = worn_laps * (worn_laps + 1.0) / 2.0
    excess_age_sq_sum = worn_laps * (worn_laps + 1.0) * (2.0 * worn_laps + 1.0) / 6.0

    return total_age_sum, fresh_age_sum, excess_age_sum, excess_age_sq_sum
...
def add_piecewise_statistics(df: pd.DataFrame, thresholds: dict) -> pd.DataFrame:
    ...
    for stint_idx in (1, 2, 3):
        ...
        for compound in COMPOUNDS:
            ...
            if not mask.any():
                continue
            total_age_sum, fresh_age_sum, excess_age_sum, excess_age_sq_sum = stint_contribution(
                laps[mask].to_numpy(dtype=float), thresholds[compound]
            )
            out.loc[mask, f"{prefix}_age_sum"] += total_age_sum
...
def sample_params(rng: np.random.Generator) -> tuple[dict, dict]:
    ...
    params = {
        "soft_offset": rng.uniform(-2.5, -0.3),
        "medium_offset": rng.uniform(-1.2, 0.8),
        "hard_offset": rng.uniform(0.1, 2.2),
    }
