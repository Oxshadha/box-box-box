...
MODEL_TAG = os.environ.get("MODEL_TAG", "comp_mixed")
...
def race_filter(frame):
    ...
    warm_extremes = (race_level["track_temp"] <= 24) | (race_level["track_temp"] >= 37)
    mixed_comp = (
        (race_level["two_stop_drivers"] >= 4)
        | (race_level["unique_sequence_count"] >= 7)
        | (
            (race_level["total_laps"] >= 48)
            & race_level["top_sequence"].isin(["MEDIUM>HARD", "MEDIUM>SOFT", "HARD>MEDIUM", "HARD>SOFT"])
        )
        | ((race_level["total_laps"] >= 46) & warm_extremes)
    )
    return set(race_level.loc[mixed_comp, "race_id"])
...
def main():
    ...
    if train["race_id"].nunique() == 0:
        raise SystemExit("No comp specialist training races matched the filter")
