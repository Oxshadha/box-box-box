...
OUTPUT_DIR = ROOT / "analysis" / "models" / "comp_tiebreaker"
...
def route_comp_races(frame):
    ...
    no_comp = (
        ...
        | (dominant_soft_hard & (race_level["two_stop"] == 0) & (race_level["unique_sequences"] <= 6) & (race_level["total_laps"] <= 55))
        | (dominant_hard_medium_short & (race_level["two_stop"] == 0) & (race_level["unique_sequences"] <= 5) & race_level["temp_band"].isin(["mild", "warm"]))
        | (dominant_medium_hard_short & (race_level["two_stop"] == 0) & (race_level["unique_sequences"] <= 5))
    )

    comp_races = set(race_level.loc[~balanced_cluster & ~no_comp, "race_id"])
    return frame[frame["race_id"].isin(comp_races)].copy()
...
def build_pair_dataset(frame):
    ...
    for _, race in frame.groupby("race_id", sort=False):
        ...
        for i, j in itertools.combinations(range(len(race)), 2):
            if abs(float(race.loc[i, "base_score"] - race.loc[j, "base_score"])) > PAIR_GAP:
                continue
            left = race.iloc[i]
            right = race.iloc[j]
...
def apply_tiebreaker(frame, model):
    ...
    for _, race in frame.groupby("race_id", sort=False):
        ...
        for i in range(len(race) - 1):
            ...
            if abs(float(race.loc[i, "base_score"] - race.loc[j, "base_score"])) > PAIR_GAP:
                continue
            pair_features.append(pair_row(race.iloc[i], race.iloc[j]))
...
def main():
    ...
    comp_meta, comp_model = load_comp_bundle()
    df = base.add_race_composition_features(df, comp_meta.get("major_sequences", []))
    encoded = encode_with_metadata(df[comp_meta["feature_columns"]], comp_meta)
    df["base_score"] = comp_model.predict(encoded)
    df = route_comp_races(df)
    ...
    METADATA_PATH.write_text(json.dumps({"pair_gap": PAIR_GAP, "adjustment_strength": ADJUSTMENT_STRENGTH}, indent=2))
